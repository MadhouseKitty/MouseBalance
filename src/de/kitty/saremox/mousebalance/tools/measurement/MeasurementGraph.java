...
public class MeasurementGraph extends JPanel
{
	...
	public void paint(Graphics g)
	{
		...
		int chartTitelX = (mySize.width - g2d.getFontMetrics().stringWidth(
				"Gewichts Entwicklung Ueber 1 Monat")) / 2;
		...
		for (int i = 0; i <= DAYSPERMONTH; i++)
		{
			if (i % 4 == 0)
			{
				g2d.drawLine((int) (chartStartX + chartXAxisLineGap * i),
						chartStartY + 10,
						(int) (chartStartX + chartXAxisLineGap * i), chartEndY);
			} else
			{
				g2d.drawLine((int) (chartStartX + chartXAxisLineGap * i),
						chartStartY + 20,
		...
		{
			g2d.drawLine(chartStartX, (int) (chartStartY - chartYAxisLineGap
					* i), chartEndX + 10,
					(int) (chartStartY - chartYAxisLineGap * i));
		}
		...
		/*
		 * Begin to Draw Graph We use the last Measurement and than get into
		 * past
		 */
		...
		double pixelBetweenPoints = (chartEndX - chartStartX)
				/ displayedMeasurePoints;
		...
		while (curItem >= 0 && x >= 0)
		{
			...
			Measurement curMeasurement = _measurements.get(curItem);
			g2d.fillOval((int) (chartStartX + pixelBetweenPoints * x) - 4,
					(int) (chartStartY - (curMeasurement.getWeight()
							.getWeight() * pixelPerGramm)) - 4, 8, 8);
			if (x < displayedMeasurePoints)
			{
				g2d.drawLine((int) (chartStartX + pixelBetweenPoints * x),
						(int) (chartStartY - (curMeasurement.getWeight()
								.getWeight() * pixelPerGramm)),
						(int) (chartStartX + pixelBetweenPoints * (x + 1)),
						(int) (chartStartY - (_measurements.get(curItem + 1)
								.getWeight().getWeight() * pixelPerGramm)));

			}
			g2d.setColor(Color.BLACK);
			if (x % (6 * this.months) == 0)
			{
				g2d.drawString(new SimpleDateFormat("dd.MM")
						.format(curMeasurement.getDate()), (float) (chartStartX
						+ (pixelBetweenPoints * x) - pixelForDate / 2),
						chartStartY + 15);
		...
		for (int i = 0; i < DAYSPERMONTH; i++)
		{
			g2d.drawString(Integer
					.toString((int) ((i * chartYAxisLineGap) / pixelPerGramm)),
					chartEndX + 10,
					(int) (chartStartY - i * chartYAxisLineGap) + 7);
		}
	}
